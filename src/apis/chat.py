...
@router.post("/generate", tags=["RAG"])
def generate_response(request: models.QueryRequest):
...
@router.get("/history/{user_id}/{session_id}", tags=["Chats"])
...
@router.get("/history/{user_id}", tags=["Chats"])
